module.exports = {
  _minSize: function () {
    if (!this.sizeLimits || !this.sizeLimits[0] || !this.canvas) return false;

    return this.sizeLimits[0] * this._canvasSize();
  },

  _maxSize: function () {
    if (!this.sizeLimits || !this.sizeLimits[1] || !this.canvas) return false;
  ...
  _limitDimension: function (x) {
    ...
    if (min !== false) {
      ...
      if (Math.abs(x) < min) return x >= 0 ? min : -min;
    }

    if (max !== false) {
      if (Math.abs(x) > max) return x >= 0 ? max : -max;
    }
    return x;
  },
